...
import numpy as np
import math
import flow_on_sphere
flow_on_sphere.config.set_general_constants(1.0e-6,100,8.0,8,0.01)
flow_on_sphere.config.set_Omega_characteristics(1,3)

flow_on_sphere.dynamics
...
wx = np.sum(Omega_magnitudes * Omega_xs)/N
wy = np.sum(Omega_magnitudes * Omega_ys)/N
wz = np.sum(Omega_magnitudes * Omega_zs)/N
...
def iterate(thetas,phies,r_vec,theta_vec,phi_vec,kt,kp):
	...
	kt[:,0],kp[:,0] = dy.F(thetas,phies,r_vec,theta_vec,phi_vec)
	kt[:,1],kp[:,1] = dy.F(thetas+0.5*dt*kt[:,0],phies+0.5*dt*kp[:,0],r_vec,theta_vec,phi_vec)
	kt[:,2],kp[:,2] = dy.F(thetas+0.5*dt*kt[:,1],phies+0.5*dt*kp[:,1],r_vec,theta_vec,phi_vec)
	kt[:,3],kp[:,3] = dy.F(thetas+dt*kt[:,2],phies+dt*kp[:,2],r_vec,theta_vec,phi_vec)
...
if c=='y':
	...
	ani.save(filename='motion2.gif',writer='ffmpeg',dpi=130,fps=20)
...
def F_toy(thetas,phies,r_vec,theta_vec,phi_vec):
	...
	theta_dot *= Kappa/N
	theta_dot += np.multiply(Omega_magnitudes,phi_vec[:,0]*Omega_xs+phi_vec[:,1]*Omega_ys+phi_vec[:,2]*Omega_zs)
	...
	phi_dot *= Kappa/N
	phi_dot -= np.multiply(Omega_magnitudes,theta_vec[:,0]*Omega_xs+theta_vec[:,1]*Omega_ys+theta_vec[:,2]*Omega_zs)
